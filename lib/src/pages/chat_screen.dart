...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    _presenceService = PresenceService(userId: widget.currentUserId);
    ...
    return StreamBuilder<List<ChatMessage>>(
      ...
      builder: (context, snapshot) {
        ...
        final filteredMessages = allMessages.where((msg) {
          // Only apply the filter if the current user is the receiver
          if (msg.receiverId == widget.currentUserId) {
            return msg.messageStatus == MessageStatus.sent;
          }
  ...
  Future<void> pickAndSendMedia(
    ...
    try {
      if (media != null) {
        messageId = await handleSendMessage(messageType: msgType);
